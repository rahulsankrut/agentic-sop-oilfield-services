...
- **Agent Engine deploy of A2aAgent — Pydantic ↔ protobuf bug (FIXED).** `vertexai._genai._agent_engines_utils._generate_class_methods_spec_or_raise` at line 636 calls `json_format.MessageToJson(agent.agent_card)`, which raises `AttributeError: 'AgentCard' object has no attribute 'DESCRIPTOR'` when the card is `a2a-sdk` 0.x's Pydantic model. The buggy code path runs only when `agent` has an `agent_card` attribute — i.e., for `A2aAgent` wrappers, not for bare `LlmAgent` deploys via `adk deploy agent_engine` CLI. **Two deploy paths:**
  1. **Standard ADK agents (no A2A on the agent itself)** — use the `adk deploy agent_engine` CLI (per https://adk.dev/deploy/agent-runtime/deploy/). Validated end-to-end in TASK-01 Step 8.
  2. **A2A-exposing agents on Agent Engine** (e.g., procurement_approval_agent in TASK-02) — use `src/utils/deploy.py:deploy_a2a_agent_engine()`, which calls `patch_message_to_json_for_pydantic()` first. Regression test in `tests/unit/test_deploy_patch.py` proves it works against the real bug.
