...
declare ``extra_packages=[agents/forecast_review_agent, agents/utils, agents/schemas.py]``
...
the standard ADK query API), so we don't need ``agents.utils.deploy``'s patch.

Usage:
    poetry run python -m agents.forecast_review_agent.deploy
"""
